...
const updateProductoNombre = async (iid_nombre, productoNombre, req) => {
...
const deleteProductoNombre = async (iid_nombre, req) => {
    return executeWithAudit(pool, req, async (client) => {
        const query = `
      UPDATE public.tbl_productos
      SET bactivo = false
      ...
      RETURNING *;
    `;
        const { rows } = await client.query(query, [iid_nombre]);
